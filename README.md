...
```
Google Drive / laptop / phone  --Drives Sync, sync/-->  Drive (private)     add-only; see Drive's sync/README.md
...
reaches Drive from Google Drive and from device folders through the Drives Sync app (`drives-sync/`
here) and Drive's `sync/` tool, both of which only ever add files (see Drive's `sync/README.md`).
...
| `drives-sync/` | **Drives Sync**, a small PWA (https://hongguqaz.github.io/Lobby/drives-sync/) that syncs Google Drive and device folders into the private Drive repository's `fin-lab/FinResearchRaw/`, one way and additively. Its `README.md` has the setup; `agent/` has the command line for agents. It holds no data and no secrets; tokens stay in the browser. |
